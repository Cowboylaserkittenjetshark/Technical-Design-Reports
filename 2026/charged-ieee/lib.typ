...
#let ieee(
  ...
  // Appendecies to be appended at the bottom of the paper
  appendices: none,
  ...
  set page(
    columns: 2,
    header: align(right)[AquaPack Robotics at NC State],
    paper: paper-size,
  ...
  place(
    ...
    clearance: 30pt,
    {
      ...
      set align(center)
      [
        #authors.map(author => author.name + super[#author.major.symbol]).join(", ")
        #linebreak()
        #authors.map(author => author.major.symbol + author.major.name).dedup().join(", ")
      ]
